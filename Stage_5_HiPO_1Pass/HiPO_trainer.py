...
from Stage_5_HiPO_1Pass.HiPO import HierarchicalPreferenceOptimization
...
def training(
    ...
    eval_metrics: dict,
    DPO: HierarchicalPreferenceOptimization, 
    loader: DataLoader, 
    gen_prompt_ids: dict, 
        ...
        for epoch in range(int(w[-1].item())):
            ...
            for batch in data_loader:
                ...
                if method == 'sft':
                    ...
                    loss, loss_components = DPO.hipo_loss(batch, Prompt_Instruction = gen_prompt_ids,beta = config_schema.beta, weights=w[:-2])
...
def init():
    ...
    DPO = HierarchicalPreferenceOptimization(config_schema.beta, DEVICE, config_schema.lr, config_schema.max_len)
