...
class DirectPreferenceOptimization:
    ...
    def set_models(self, MODEL_NAME):
        ...
        self.ref_model = AutoModelForCausalLM.from_pretrained(MODEL_NAME, torch_dtype=torch.float16).to(self.device)
        self.ref_model.eval()  # Reference model should be in eval mode
        for param in self.ref_model.parameters():
            param.requires_grad = False  # Freeze reference model

        self.policy_model = AutoModelForCausalLM.from_pretrained(MODEL_NAME, torch_dtype=torch.float16).to(self.device)
